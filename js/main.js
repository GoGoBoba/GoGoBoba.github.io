...
const imgReferenceMap = new Map();
function findParent(element, className, limit) {
    var pn = element;
    for(i = 0; i < limit; i++) {
...
function clickedThumbnail(img) {
    let mainImg = null;
    if(imgReferenceMap.has(img))
    {
        mainImg = imgReferenceMap.get(img);
    }
    else
    {
        var commonAncestor = findParent(img, "daphoto-container", 5);
        mainImg = commonAncestor.querySelector('.daphoto-main');
        imgReferenceMap.set(img, mainImg);
    }

    if(mainImg != null && mainImg.src != img.src)
        mainImg.src = img.src;
 }